...
typedef BigIntTiny extll;
int n,k;
char nuyn[50];
int nums[50];
extll ans = 0;
void print_ext(extll x);
void dfs(int depth,int start,extll now);
int main(){
  scanf("%d%d",&n,&k);
  scanf("%s",nuyn);
  for(int i = 0;i < n;i++){
  ...
  dfs(0,0,(extll)1);
  ans = ans / BigIntTiny(10);
  cout << ans.to_str() << endl;
  return 0;
}
// void print_ext(extll x){
//   if(x > 9) print_ext(x / 10);
//   putchar(x % 10 + '0');
// }
void dfs(int depth,int start,extll now){
  if(depth == k + 1){
  ...
  for(int tend = start;tend <= n;tend++){
    extll tmp = BigIntTiny(0);
    for(int j = start;j <= tend;j++){
      tmp  = tmp +  BigIntTiny(nums[j]);
      tmp  = tmp * BigIntTiny(10);
    }
    tmp  = tmp / BigIntTiny(10);
    dfs(depth + 1,tend + 1,tmp * now);
  }
}
